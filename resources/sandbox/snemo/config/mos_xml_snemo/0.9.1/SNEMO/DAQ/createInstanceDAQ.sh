...
usage() {
        ...
        echo "(script) *   <Address> --> (in)  : address of the DAQMw program "
        echo "(script) *   <Port>    --> (in)  : port number of the DAQMw Program"
...
m_name=DAQdevice_$3.xml
m_modelName=DAQModel.xml
NOW=$(date +"%d-%m-%Y %k:%M")
