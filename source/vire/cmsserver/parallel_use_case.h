//! \file  vire/cmsserver/parallel_use_case.h
//! \brief Vire CMS parallel use case
//
...
#ifndef VIRE_CMSSERVER_PARALLEL_USE_CASE_H
#define VIRE_CMSSERVER_PARALLEL_USE_CASE_H
...
namespace vire {
  ...
  namespace cmsserver {
    ...
    /// \brief Parallel agregator use case which runs of set of N daughter use cases
    ///        in parallel for a given period.
    ///
    /// Time layout:
    /// \code
    /// 
    ...
    /// \encode
    ///
    class parallel_use_case
      : public base_use_case
    {
    public:
    
      //! Default constructor
      parallel_use_case(const uint32_t flags_ = 0);

      //! Destructor
      virtual ~parallel_use_case();
...
#endif // VIRE_CMSSERVER_PARALLEL_USE_CASE_H
